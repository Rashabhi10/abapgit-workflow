CLASS yrv_fill_flight DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS YRV_FILL_FLIGHT IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    DATA:itab TYPE TABLE OF yrv_travel_tab.
...
*   delete existing entries in the database table
    DELETE FROM yrv_travel_tab.

*   insert the new table entries
    INSERT yrv_travel_tab FROM TABLE @itab.

*   check the result
    SELECT * FROM yrv_travel_tab INTO TABLE @itab.
    out->write( sy-dbcnt ).
